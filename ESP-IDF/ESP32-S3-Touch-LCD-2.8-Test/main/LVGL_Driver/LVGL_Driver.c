...
static const uint32_t LVGL_DRAW_BUFFER_CAPS = MALLOC_CAP_INTERNAL | MALLOC_CAP_DMA | MALLOC_CAP_8BIT;
static const uint32_t LVGL_DRAW_BUFFER_FALLBACK_CAPS = MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT;
...
static void *alloc_lvgl_buffer(size_t buffer_size, const char *buffer_name)
{
    void *buffer = heap_caps_malloc(buffer_size, LVGL_DRAW_BUFFER_CAPS);

    if (buffer != NULL) {
        ESP_LOGI(TAG_LVGL, "Allocated %s in internal DMA RAM (%u bytes)", buffer_name, (unsigned int)buffer_size);
        return buffer;
    }

    buffer = heap_caps_malloc(buffer_size, LVGL_DRAW_BUFFER_FALLBACK_CAPS);
    if (buffer != NULL) {
        ESP_LOGW(TAG_LVGL, "Allocated %s in PSRAM fallback (%u bytes)", buffer_name, (unsigned int)buffer_size);
        return buffer;
    }

    ESP_LOGE(TAG_LVGL, "Failed to allocate %s (%u bytes)", buffer_name, (unsigned int)buffer_size);
    return NULL;
}
...
void LVGL_Init(void)
{
    size_t draw_buffer_size = LVGL_BUF_LEN * sizeof(lv_color_t);

    ESP_LOGI(TAG_LVGL, "Initialize LVGL library");
    lv_init();
    
    buf1 = alloc_lvgl_buffer(draw_buffer_size, "buf1");
    assert(buf1);
    buf2 = alloc_lvgl_buffer(draw_buffer_size, "buf2");
    assert(buf2);
